...
/*   Updated: 2015/01/13 06:11:57 by ebaudet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		RobotomyRequestForm::execute(Bureaucrat const& executor) const {
	try {
		...
		std::cout << "\a * Rizzzzzzzzzzzz * " << std::endl;
		if (rand() % 2)
			std::cout << this->_target << "'s robotomy was a success!" << std::endl;
		else
			std::cout << "We are afraid " << this->_target << "'s robotomy was a failure. Here are \"it's\" ashes.." << std::endl;
